...
from app.forms import (RegistrationForm,LoginForm ,UpdateForm,PostForm,RequestResetForm,ResetPasswordForm)#we import the forms we created in forms.py so that we can use them in out routes
...
def home():
    ...
    page=request.args.get('page',1,type=int) #this is to target the page number from the url and if there is no page no provided it will default to 1
...
def reset_token(token):
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
        flash('your password has been updated','success') #success is a category for the flash message
        return redirect(url_for('login'))
